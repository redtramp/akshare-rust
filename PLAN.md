...
> 口径：以 akshare `akshare/__init__.py` 实际导出的**公开 API 名**为准（AST 解析去重 = **1094** 个，与 PLAN 目标 1099 基本一致）；Rust 侧以各公开函数 doc comment「对应 akshare `akshare.X`」声明的 **150** 个为准，并逐一与 akshare 公开名交叉验证（全部命中，无虚报）。

| 指标 | 数值 |
|---|---|
| akshare 公开 API 总数 | **1094** |
| Rust 已实现并验证函数数 | **150**（`cargo check` 全绿，非 stub）|
| 整体覆盖率 | **≈ 13.7%** |
...
**结论：** 已完成一条纵深的「样板通路」（8 类数据源 + 完整管线），约覆盖 13.7% 公开函数、集中在 stock/fund/index/stock_fundamental/futures。距 1099 全量目标，剩余 ~86% 主要是**同类数据源下的广度扩展**（economic 226、stock_feature 余 123、futures 余 63/option 47/bond 46），底层能力基本已就位，属可批量推进区间。
