...
| fund | 5 | 74 | 6.8% |
| index | 3 | 79 | 3.8% |
| stock_feature | 74 | 211 | 35.1% |

**完全未覆盖大类（0%）：** economic(226)、stock_feature(余 137)、futures(70)、stock_fundamental(57)、option(47)、bond(46)，以及其余长尾 24 类（spot/futures_derivative/movie/energy/currency/news/fx/fortune/cal/qdii/reits/event/forex/crypto/rate/nlp/tool/hf/interest_rate/bank/pro/other/article/air/qhkc_web）全部为 0。
...
> - **批次 1 · 后续阶段**：stock_feature 其余东财 datacenter 函数（分析师 other-host `data.eastmoney.com/dataapi`：`stock_analyst_rank_em`/`stock_analyst_detail_em`；新股申购多分支 `stock_xgsglb_em`），以及同花顺 `ths.js` 系、乐咕/新浪系。
