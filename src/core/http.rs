...
impl HttpClient {
    ...
    /// 带重试的 GET（自定义请求头），返回**原始字节**。
    ///
    /// 用于二进制响应（如郑商所 `.xls` 仓单/交割文件、中金所 GBK 编码 `.csv`），
    /// 这些响应不能经 `decode_body` 的字符集启发式解码（会损坏二进制）。
    /// 重试策略与 `get_text_with_headers` 一致：仅对 5xx 与连接错误重试，4xx 立即返回。
    ...
    /// 带重试的 POST（JSON 请求体 + 自定义请求头），返回解析后的 JSON。
    ///
    /// 对应 akshare `requests.post(url, json=payload)`：东方财富 `emappdata.eastmoney.com`
    /// 等人气榜/资金流接口要求 JSON 体（而非 query 参数）。重试策略与 GET 一致：
    /// 仅对 5xx 与连接错误重试，4xx 立即返回。
    pub fn post_json_body(
        &self,
        url: &str,
        body: &Value,
        headers: &[(&str, &str)],
